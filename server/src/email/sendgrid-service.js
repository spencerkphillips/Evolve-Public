...
const sendPasswordResetEmail = async (to, subject, token) => {
	let recipient = to;
	if (to.endsWith(process.env.APP_HOST)) {
		recipient = process.env.DEFAULT_EMAIL_REIEVER;
	}

	const msg = {
		to: recipient,
		from: process.env.EMAIL_SENDER || 'hello@reviveweb.co',
		subject,
		text: token,
...
const sendPasswordResetSuccessfulEmail = async (to, subject) => {
	...
	const msg = {
		...
		from: process.env.EMAIL_SENDER || 'hello@reviveweb.co', // Change to your verified sender
		subject,
