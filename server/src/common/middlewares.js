...
const handleRequest = async (req, res, next) => {
	...
	res.log = req.log.child({ correlationId });
	res.log.info(`new request: ${req.method} ${req.url}`);
	return next();
};
	...
	if (auth) {
		...
		jwt.verify(auth, process.env.JWT_SECRET, (err, decoded) => {
			if (err) {
				...
				req.log.info(`Authentication user ${req.user.username}`);
				next();
			}
		});
	} else {
